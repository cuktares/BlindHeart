...
public class TargetDetectionControl : MonoBehaviour
{
    ...
    Transform GetClosestEnemyInDirection(Vector3 inputDirection)
    {
        ...
        allTargetsInScene.RemoveAll(enemy => enemy == null);
        ...
        foreach (Transform enemy in allTargetsInScene)
        {
            ...
            if (enemyBase != null && enemyBase.isDead) continue; // Skip dead enemies
    ...
    {
        if (allTargetsInScene.Contains(target))
        {
            allTargetsInScene.Remove(target);
            if (debug)
